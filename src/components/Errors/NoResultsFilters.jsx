function NoResultsFilters({onClear}) {
  return (
    <div className="flex flex-col items-center justify-start min-h-screen">
      ...
      <h1 className="text-3xl mb-12 mt-2 font-bold text-[#2A6A90] text-center font-serif">
      ...
      <button 
        onClick={onClear}
        type="button" 
        className="bg-[#2A6A9066] rounded-full w-50 h-20  cursor-pointer flex items-center justify-center gap-3 px-4"
      >
        <span className="text-xl font-medium text-[#2A6A90]">
          Limpar Filtros
        </span>
         <svg xmlns="http://www.w3.org/2000/svg" 
            className="w-6 h-6  text-[#2A6A90]"
            height="24px" 
            viewBox="0 -960 960 960" 
            width="24px" 
            fill="currentColor">
